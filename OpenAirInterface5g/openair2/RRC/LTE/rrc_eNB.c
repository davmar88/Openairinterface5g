...
//-----------------------------------------------------------------------------
{
  ...
  if (EPC_MODE_ENABLED == 1) {
    ...
    //rrc_eNB_generate_UECapabilityEnquiry (&ctxt_pP, ue_context_pP);//Davel remove line
  }
}
...
//-----------------------------------------------------------------------------
{
  ...
  LOG_DUMPMSG(RRC,DEBUG_RRC,(char *)buffer,size,"\033[1;34mbold [MSG] RRC Security Mode Command\033[0m\n");
  LOG_I(RRC,
        PROTOCOL_RRC_CTXT_UE_FMT"\033[1;34mbold Logical Channel DL-DCCH, Generate SecurityModeCommand (bytes %d)\033[0m\n",
        PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
        size);
  LOG_D(RRC,
        PROTOCOL_RRC_CTXT_UE_FMT"\033[1;34mbold --- PDCP_DATA_REQ/%d Bytes (securityModeCommand to UE MUI %d) --->[PDCP][RB %02d]\033[0m\n",
        PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
        size,
        rrc_eNB_mui,
        DCCH);
  MSC_LOG_TX_MESSAGE(
    ...
    buffer,
    size,
    MSC_AS_TIME_FMT"\033[1;34mbold securityModeCommand UE %x MUI %d size %u \033[0m\n",
  ...
  if (!NODE_IS_DU(RC.rrc[ctxt_pP->module_id]->node_type)) {
    LOG_I(RRC,"\033[1;34mbold calling rrc_data_req :securityModeCommand \033[0m\n");
...
//-----------------------------------------------------------------------------
{
  ...
  size = do_UECapabilityEnquiry(
           ...
           rrc_eNB_get_next_transaction_identifier(ctxt_pP->module_id));
  LOG_I(RRC,
        PROTOCOL_RRC_CTXT_UE_FMT"\033[1;33mbold Logical Channel DL-DCCH, Generate UECapabilityEnquiry (bytes %d)\033[0m\n",
        PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
        size);
  LOG_D(RRC,
        PROTOCOL_RRC_CTXT_UE_FMT"\033[1;33mbold --- PDCP_DATA_REQ/%d Bytes (UECapabilityEnquiry MUI %d) --->[PDCP][RB %02d] \033[0m\n",
        PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
        size,
        rrc_eNB_mui,
        DCCH);
  MSC_LOG_TX_MESSAGE(
    ...
    buffer,
    size,
    MSC_AS_TIME_FMT"\033[1;32mbold rrcUECapabilityEnquiry UE %x MUI %d size %u \033[0m\n",
    MSC_AS_TIME_ARGS(ctxt_pP),
    ue_context_pP->ue_context.rnti,
    rrc_eNB_mui,
    size);
...
//-----------------------------------------------------------------------------
{
  ...
  *DRB_configList2 = CALLOC(1, sizeof(**DRB_configList2));
  ...
  //Davel added ue_context_pP to do_RRCConnectionReconfiguration
...
//#endif
  ...
  if(size==65535) {
    ...
    //Davel ---------Generate LTE_RRCConnectionReconfiguration
    LOG_I(RRC,
          "[eNB %d] Frame %d, Logical Channel DL-DCCH, Generate LTE_RRCConnectionReconfiguration INSIDE rrc_eNB_process_RRCConnectionReestablishmentComplete (bytes %d, UE id %x)\n",
...
//-----------------------------------------------------------------------------
{
  ...
  *DRB_configList2 = CALLOC(1, sizeof(**DRB_configList2));
  ...
  LOG_I(RRC,
        "[eNB %d] Frame %d, Logical Channel DL-DCCH, Generate LTE_RRCConnectionReconfiguration INSIDE rrc_eNB_generate_dedicatedRRCConnectionReconfiguration (bytes %d, UE RNTI %x)\n",
        ctxt_pP->module_id, ctxt_pP->frame, size, ue_context_pP->ue_context.rnti);
  LOG_D(RRC,
        "[FRAME %05d][RRC_eNB][MOD %u][][--- PDCP_DATA_REQ/%d Bytes (rrcConnectionReconfiguration to UE %x MUI %d) --->][PDCP][MOD %u][RB %u]\n",
        ctxt_pP->frame, ctxt_pP->module_id, size, ue_context_pP->ue_context.rnti, rrc_eNB_mui, ctxt_pP->module_id, DCCH);
...
//-----------------------------------------------------------------------------
{
  ...
  LOG_I(RRC,
        "[eNB %d] Frame %d, Logical Channel DL-DCCH, Generate LTE_RRCConnectionReconfiguration INSIDE rrc_eNB_modify_dedicatedRRCConnectionReconfiguration (bytes %d, UE RNTI %x)\n",
        ctxt_pP->module_id, ctxt_pP->frame, size, ue_context_pP->ue_context.rnti);
  LOG_D(RRC,
        "[FRAME %05d][RRC_eNB][MOD %u][][--- PDCP_DATA_REQ/%d Bytes (rrcConnectionReconfiguration to UE %x MUI %d) --->][PDCP][MOD %u][RB %u]\n",
        ctxt_pP->frame, ctxt_pP->module_id, size, ue_context_pP->ue_context.rnti, rrc_eNB_mui, ctxt_pP->module_id, DCCH);
...
//-----------------------------------------------------------------------------
{
  ...
  *DRB_Release_configList2 = CALLOC(1, sizeof(**DRB_Release_configList2));
  ...
  LOG_I(RRC,
        "[eNB %d] Frame %d, Logical Channel DL-DCCH, Generate LTE_RRCConnectionReconfiguration INSIDE  (bytes %d, UE RNTI %x)\n",
        ctxt_pP->module_id, ctxt_pP->frame, size, ue_context_pP->ue_context.rnti);
  LOG_D(RRC,
        "[FRAME %05d][RRC_eNB][MOD %u][][--- PDCP_DATA_REQ/%d Bytes (rrcConnectionReconfiguration to UE %x MUI %d) --->][PDCP][MOD %u][RB %u]\n",
        ctxt_pP->frame, ctxt_pP->module_id, size, ue_context_pP->ue_context.rnti, rrc_eNB_mui, ctxt_pP->module_id, DCCH);
...
//-----------------------------------------------------------------------------
{
  ...
  *quantityConfig->quantityConfigEUTRA->filterCoefficientRSRQ = LTE_FilterCoefficient_fc4;
  ...
  //DAVEL FOCUS ON THIS METHOD!!!!
  LOG_I(RRC,"MEASUREMENT ENABLED: %d \n",measurements_enabled);//Davel remove line
  ...
  LOG_I(RRC, "[eNB %d] Frame %d, Logical Channel DL-DCCH, Generate LTE_RRCConnectionReconfiguration INSIDE rrc_eNB_generate_defaultRRCConnectionReconfiguration (bytes %d, UE id %x)\n",
        ctxt_pP->module_id, 
        ctxt_pP->frame, 
        size, 
        ue_context_pP->ue_context.rnti);
...
LOG_I(RRC, "\033[1;33mbold INSIDE MEASUREMENT REPORT!!!!!!!!!!!!!\033[0m\n");
/*MYSQL mysql;
MYSQL_RES *result;
MYSQL_ROW row;*/

MYSQL *conn, *conn2; /* pointer to connection handler */
MYSQL_RES *res,*res2; /* holds the result set */
MYSQL_ROW row,row2;
...
char   query[1000];
char   query2[1000];
//LOG_I(RRC, "\033[1;32mbold INSIDE MEASUREMENT REPORT!!!!!!!!!!\033[0m\n");
int query_state;
LOG_I(RRC, "\033[1;33mbold PASSED int query state\033[0m\n");
  int i=0;
  ...
  {
    LOG_I(RRC, "\033[1;33mbold measResults2 is NULL\033[0m\n");
    return;
  }
  else 
  {
    LOG_I(RRC, "\033[1;33mbold measResults2 is NOT NULL \033[0m\n");
  }
    


  if (measResults2->measId > 0 ) {
    LOG_I(RRC, "\033[1;33mbold measResults2->measId is greater than 0 \033[0m\n");
...
//measResults2->measId
    switch (2) {
      case 1:
        LOG_D(RRC,"\033[1;32mbold Periodic report at frame %d and subframe %d \033[0m\n", ctxt_pP->frame, ctxt_pP->subframe);
        break;

      case 2:
        LOG_D(RRC,"\033[1;32mbold A1 event report (Serving becomes better than absolute threshold) at frame %d and subframe %d \033[0m\n", ctxt_pP->frame, ctxt_pP->subframe);
        break;

      case 3:
        LOG_D(RRC,"\033[1;32mbold A2 event report (Serving becomes worse than absolute threshold) at frame %d and subframe %d \033[0m\n", ctxt_pP->frame, ctxt_pP->subframe);
        break;

      case 4:
        LOG_D(RRC,"\033[1;32mbold A3 event report (Neighbour becomes amount of offset better than PCell) at frame %d and subframe %d \033[0m\n", ctxt_pP->frame, ctxt_pP->subframe);
        break;

      case 5:
        LOG_D(RRC,"\033[1;32mbold A4 event report (Neighbour becomes better than absolute threshold) at frame %d and subframe %d \033[0m\n", ctxt_pP->frame, ctxt_pP->subframe);
        break;

      case 6:
        LOG_D(RRC,"\033[1;32mbold A5 event report (PCell becomes worse than absolute threshold1 AND Neighbour becomes better than another absolute threshold2) at frame %d and subframe %d \033[0m\n", ctxt_pP->frame,
              ctxt_pP->subframe);
        break;

      default:
        LOG_D(RRC,"\033[1;32mbold Other event report frame %d and subframe %d \033[0m\n", ctxt_pP->frame, ctxt_pP->subframe);
        break;
    }
    ...
    LOG_I(RRC, "\033[1;32mbold [eNB %d]Frame %d: UE %x (Measurement Id %d): RSRP of Source %ld \033[0m\n", ctxt_pP->module_id, ctxt_pP->frame, ctxt_pP->rnti, (int)measResults2->measId,
          ue_context_pP->ue_context.measResults->measResultPCell.rsrpResult-140);
    LOG_I(RRC, " \033[1;32mbold [eNB %d]Frame %d: UE %x (Measurement Id %d): RSRQ of Source %ld \033[0m\n", ctxt_pP->module_id, ctxt_pP->frame, ctxt_pP->rnti, (int)measResults2->measId,
          ue_context_pP->ue_context.measResults->measResultPCell.rsrqResult/2 - 20);

    if((ue_context_pP->ue_context.measResults->measResultPCell.rsrpResult-140)>10)
    {
         LOG_I(RRC, "\033[1;33mbold RSRP TOO BIG TO BE CAPTURED OF OWN TOWER, SKIPPING INSERT \033[0m\n");
    }
    else
    {
         LOG_I(RRC, "\033[1;33mbold RSRP JUST THE RIGHT SIZE OWN TOWER, INSERTING INTO DATABASE! \033[0m\n");
    ...
    mysql_query(conn, "SELECT imsi FROM Observations ORDER BY id DESC LIMIT 1");
    ...
    while ((row = mysql_fetch_row(res)))
    {
       ...
       for(int i = 0; i < num_fields; i++)
       {
           ...
           if(row[i] != NULL)
           {
             ...
             LOG_I(RRC, "\033[1;33mbold IMSI: %s \033[0m\n",row[i]);
              
                sprintf (query, "INSERT into `oai_db`.`Measurementreports` (`cellid`, `rsrp`,`imsi`,`sent`) VALUES ('%d','%ld','%s','%d')", 0,ue_context_pP->ue_context.measResults->measResultPCell.rsrpResult-140,row[i],0);
           ...
           {
             LOG_I(RRC, "\033[1;33mbold Could NOT FIND IMSI \033[0m\n");
           }
  ...
  {
      LOG_I(RRC, "\033[1;33mbold measResults2->measId is 0 \033[0m\n");
  }
  ...
  {
    LOG_I(RRC, "\033[1;33mbold measResults2->measResultNeighCells is NULL\033[0m\n");
      return; //gives signal 11 if I force it 
  }
  else
  {
    LOG_I(RRC, "\033[1;33mbold measResults2->measResultNeighCells is NOT NULL\033[0m\n");
  }
  
    

  if (measResults2->measResultNeighCells->choice.measResultListEUTRA.list.count > 0) {
    LOG_I(RRC, "\033[1;33mbold measResultListEUTRA.list.count is greater than 0 \033[0m\n");
    ...
    for (i=0; i < neighboring_cells; i++) {
      LOG_I(RRC, "\033[1;33mbold Iterating through neighboring cells \033[0m\n");
      ...
      LOG_I(RRC, "\033[1;32mbold RSRQ of Target %ld \033[0m\n",
      ...
      int physcellid = (int)ue_context_pP->ue_context.measResults->measResultNeighCells->choice.measResultListEUTRA.list.array[i]->physCellId;

      //char TRAC [1000];
      int TRAC = (int)ue_context_pP->ue_context.measResults->measResultNeighCells->choice.measResultListEUTRA.list.array[i]->_asn_ctx.context;//Davel trying to get TAC
      LOG_I(RRC, "\033[1;32mbold TRACKING AREA CODE %d"" \033[0m\n",TRAC);
        /* INITIALIZE CONNECTION HANDLER, DO NOT CHANGE */
        ...
        for(int i = 0; i < num_fields; i++)
        {
            ...
            if(row[i] != NULL)
            {
              ...
              LOG_I(RRC, "\033[1;33mbold IMSI: %s \033[0m\n",row[i]);
                  ...
                  sprintf (query, "INSERT into `oai_db`.`Measurementreports` (`cellid`, `rsrp`,`imsi`,`sent`) VALUES ('%d','%ld','%s','%d')", physcellid,(*ue_context_pP->ue_context.measResults->measResultNeighCells->choice.measResultListEUTRA.list.array[i]->measResult.rsrpResult)-140,row[i],0);
                  mysql_query(conn, query);
  ...
  {
    LOG_I(RRC, "\033[1;33mbold measResultListEUTRA.list.count is 0 \033[0m\n");
  }
      ...
      case LTE_UL_CCCH_MessageType__c1_PR_rrcConnectionReestablishmentRequest:
               ...
               "reconfigurationFailure"));
        {
          ...
          if (rrcConnectionReestablishmentRequest->ue_Identity.physCellId != RC.rrc[ctxt_pP->module_id]->carrier[CC_id].physCellId) {
            ...
            //rrc_eNB_generate_RRCConnectionReestablishmentReject(ctxt_pP, ue_context_p, CC_id); //Davel remove line
            break;
          }
        ...
        break;
          ...
          if(ue_context_p->ue_context.ue_reestablishment_timer > 0) {
            ...
            rrc_eNB_process_RRCConnectionReconfigurationComplete(&ctxt_old_p,
                ...
                //Davel ----------------------rrc_eNB_process_RRCConnectionReconfigurationComplete
  ...
  if (ul_dcch_msg->message.present == LTE_UL_DCCH_MessageType_PR_c1) {
    switch (ul_dcch_msg->message.choice.c1.present) {
      ...
      case LTE_UL_DCCH_MessageType__c1_PR_measurementReport:
        ...
        //-------------Davel----------Example how to Calling Measurement Report, do not remove-----
        LOG_D(RRC,
      ...
      case LTE_UL_DCCH_MessageType__c1_PR_rrcConnectionReconfigurationComplete:
        ...
        //Davel ------UE Capabilities Calling Measurement Report-----------
        /*LOG_D(RRC,
              PROTOCOL_RRC_CTXT_UE_FMT" RLC RB %02d --- RLC_DATA_IND "
              "%d bytes (measurementReport) ---> RRC_eNB\n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
          ...
          if(&ul_dcch_msg->message.choice.c1.choice.measurementReport.criticalExtensions.choice.c1.choice.measurementReport_r8.measResults==NULL)
          {
            LOG_I(RRC, "\033[1;33mbold measResults is Null\033[0m\n");
          }
          else
          {
            LOG_I(RRC, "\033[1;33mbold measResults is not Null\033[0m\n");
          }


        LOG_I(RRC, "\033[1;33mbold GOING TO CALL PROCESS MEASUREMENT REPORT NOW!!!!!!!!!!!!!\033[0m\n");
        ...
        /*rrc_eNB_process_MeasurementReport(
         ...
         // catch (int e)
         // {
          //  LOG_I(RRC, "\033[1;33mbold trycatch process_MeasurementReport: %d \033[0m\n",e);
      ...
      case LTE_UL_DCCH_MessageType__c1_PR_securityModeComplete:
        ...
        rrc_eNB_generate_UECapabilityEnquiry(
          ...
          //-------------Davel--------Calling Measurement Report-------
       /*LOG_D(RRC,
              PROTOCOL_RRC_CTXT_UE_FMT" RLC RB %02d --- RLC_DATA_IND "
              "%d bytes (measurementReport) ---> RRC_eNB\n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
      ...
      case LTE_UL_DCCH_MessageType__c1_PR_ueCapabilityInformation:
        ...
        LOG_I(RRC, "\033[1;33mbold INSIDE UE CAPABILITY INFORMATION!!!!!!!!!!!!!\033[0m\n");
       ...
       /* LOG_I(RRC, "\033[1;33mbold GOING TO CALL PROCESS MEASUREMENT REPORT NOW!!!!!!!!!!!!!\033[0m\n");   
        ...
        //Davel place this rather after defaultRRCConnectionReconfiguration just before we process RRCConnectionReconfigurationComplete
        rrc_eNB_process_MeasurementReport(
          ctxt_pP,
          ue_context_p,
          &ul_dcch_msg->message.choice.c1.choice.measurementReport.
