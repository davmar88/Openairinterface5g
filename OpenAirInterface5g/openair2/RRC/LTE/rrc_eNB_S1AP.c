...
void extract_imsi(uint8_t *pdu_buf, uint32_t pdu_len, rrc_eNB_ue_context_t *ue_context_pP) {
  ...
  * bearer identity, and protocol discriminator) */
  ...
  if (emm_header->message_type == IDENTITY_RESPONSE) {
    LOG_D(RRC, "\033[1;32mbold INSIDE IDENTITY RESPONSE \033[0m\n");
    ...
    LOG_D(RRC, "\033[1;32mbold INSIDE ATTACH REQUEST \033[0m\n");
    ...
    if (e_msg->attach_request.oldgutiorimsi.imsi.typeofidentity == MOBILE_IDENTITY_IMSI) {
      LOG_D(RRC, "\033[1;32mbold INSIDE ATTACH REQUEST: MOBILE_IDENTITY_IMSI \033[0m\n");
...
//------------------------------------------------------------------------------
{
  eNB_RRC_INST *rrc = RC.rrc[ctxt_pP->module_id];
  {
    ...
    /* Fill UE identities with available information */
    {
      ...
      if (rrcConnectionSetupComplete->registeredMME != NULL) {
        ...
        MSC_LOG_TX_MESSAGE(MSC_S1AP_ENB,
                           ...
                           ctxt_pP->module_id,
                           ctxt_pP->rnti);
        // this gets generated everytime an imsi comes in...
        LOG_I(S1AP, "\033[1;32mbold [eNB %d] Build S1AP_NAS_FIRST_REQ adding in s_TMSI: GUMMEI mme_code %u mme_group_id %u ue %x\033[0m\n",
              ctxt_pP->module_id,
    ...
    //rrc_eNB_generate_UECapabilityEnquiry(ctxt_pP, ue_context_pP);//Davel remove line it works,
    rrc_eNB_generate_SecurityModeCommand(ctxt_pP, ue_context_pP); //needs this to be able to generate measurement report!!
  ...
  if (ue_context_p == NULL) {
      ...
      S1AP_INITIAL_CONTEXT_SETUP_REQ(msg_p).security_key);
    {
      ...
      if (send_security_mode_command) {
        ...
        rrc_eNB_generate_SecurityModeCommand ( //Davel uncommment
          &ctxt,
          ue_context_p);
        send_security_mode_command = FALSE;
        ...
        rrc_eNB_generate_UECapabilityEnquiry (&ctxt, ue_context_p);//Davel remove line
